...
import { LineIcon } from '@tldr/components/Primitives/icons';
import { useTldrawApp } from '@tldr/hooks';
import { TDShapeType, TDSnapshot, TDToolType } from '@tldr/types';
    ...
    <DropdownMenu.Root dir="ltr" onOpenChange={selectShapeTool}>
      ...
      <DropdownMenu.Content asChild side={contentSide} sideOffset={12}>
        <Panel side="center" style={{ flexDirection: panelStyle }}>
          {shapeShapes.map((shape, index) => (
            <Tooltip key={shape} label={intl.formatMessage({ id: shape })} kbd={(4 + index).toString()} id={`TD-PrimaryTools-Shapes-${shape}`}>
