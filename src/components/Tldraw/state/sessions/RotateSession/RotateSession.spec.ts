...
describe('Rotate session', () => {
  ...
  it('cancels session', () => {
    app.loadDocument(mockDocument).select('rect1').pointBoundsHandle('rotate', { x: 50, y: 0 }).movePointer([100, 50]).cancel();
  ...
  describe('when rotating a single shape while pressing shift', () => {
    ...
    it('Clamps rotation to 15 degrees when starting from a rotation', () => {
      ...
      app.loadDocument(mockDocument).select('rect1').pointBoundsHandle('rotate', { x: 0, y: 0 }).movePointer([5, 5]).completeSession();

      // Rect 1 clamp rotated, starting from a little rotation
      app.select('rect1').pointBoundsHandle('rotate', { x: 0, y: 0 }).movePointer({ x: 100, y: 200, shiftKey: true }).completeSession();

      expect(Math.round((app.getShape('rect1').rotation || 0) * (180 / Math.PI)) % 15).toEqual(0);

      // Try again, too.
      app.select('rect1').pointBoundsHandle('rotate', { x: 0, y: 0 }).movePointer({ x: -100, y: 5000, shiftKey: true }).completeSession();
  ...
  describe('when rotating multiple shapes', () => {
    it('keeps the center', () => {
      ...
      const centerBefore = Vec.toFixed(Utils.getBoundsCenter(Utils.getCommonBounds(app.selectedIds.map((id) => app.getShapeBounds(id)))));

      app.pointBoundsHandle('rotate', { x: 50, y: 0 }).movePointer([100, 50]).completeSession();

      const centerAfterA = Vec.toFixed(Utils.getBoundsCenter(Utils.getCommonBounds(app.selectedIds.map((id) => app.getShapeBounds(id)))));

      app.pointBoundsHandle('rotate', { x: 100, y: 0 }).movePointer([50, 0]).completeSession();

      const centerAfterB = Vec.toFixed(Utils.getBoundsCenter(Utils.getCommonBounds(app.selectedIds.map((id) => app.getShapeBounds(id)))));
