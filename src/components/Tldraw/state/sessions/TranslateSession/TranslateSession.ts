...
import { ArrowBinding, ArrowShape, GroupShape, Patch, SessionType, TDBinding, TDShape, TDShapeType, TDStatus, TldrawCommand, TldrawPatch } from '@tldr/types';
...
export class TranslateSession extends BaseSession {
  ...
  constructor(app: TldrawApp, isCreate = false, link: 'left' | 'right' | 'center' | false = false) {
    ...
    const selectedShapes = (link ? TLDR.getLinkedShapeIds(this.app.state, currentPageId, link, false) : selectedIds)
    ...
    if (((isSnapping && !metaKey) || (!isSnapping && metaKey)) && this.speed * camera.zoom < SLOW_SPEED && this.snapInfo.state === 'ready') {
      const snapResult = Utils.getSnapPoints(
        Utils.getBoundsWithCenter(
          showGrid ? Utils.snapBoundsToGrid(Utils.translateBounds(initialCommonBounds, delta), currentGrid) : Utils.translateBounds(initialCommonBounds, delta),
        ),
  ...
  cancel = (): TldrawPatch | undefined => {
    ...
    if (this.isCreate) {
      ...
      initialShapes.forEach(
        ({ id, point, handles }) => (nextShapes[id] = handles ? ({ ...nextShapes[id], point, handles } as any) : { ...nextShapes[id], point }),
      );
  ...
  private readonly createCloneInfo = () => {
