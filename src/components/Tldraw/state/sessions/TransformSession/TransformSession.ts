...
export class TransformSession extends BaseSession {
  ...
  constructor(app: TldrawApp, public transformType: TLBoundsEdge | TLBoundsCorner = TLBoundsCorner.BottomRight, public isCreate = false) {
    super(app);
    ...
    const boundsArray = Object.values(shapesBounds);

    this.initialCommonBounds = Utils.getCommonBounds(boundsArray);

    const initialInnerBounds = Utils.getBoundsFromPoints(boundsArray.map(Utils.getBoundsCenter));
  ...
  update = (): TldrawPatch | undefined => {
    ...
    let newBounds = Utils.getTransformedBoundingBox(initialCommonBounds, transformType, delta, 0, shiftKey || isAllAspectRatioLocked);
