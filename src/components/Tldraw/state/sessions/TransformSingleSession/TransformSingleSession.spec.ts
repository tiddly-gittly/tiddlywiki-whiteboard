...
describe('Transform single session', () => {
  it('begins, updateSession', () => {
    const app = new TldrawTestApp().loadDocument(mockDocument).select('rect1').pointBoundsHandle(TLBoundsCorner.TopLeft, { x: -10, y: -10 }).stopPointing();
  ...
  it('cancels session', () => {
    const app = new TldrawTestApp().loadDocument(mockDocument).select('rect1').pointBoundsHandle(TLBoundsCorner.TopLeft, { x: 5, y: 5 }).cancelSession();
...
describe('When holding alt', () => {
  it('resizes edge from center', () => {
    ...
    app.select('rect1').pointBoundsHandle(TLBoundsEdge.Left, { x: 0, y: 0 }).movePointer({ x: 20, y: 10, altKey: true }).completeSession();
  ...
  it('resizes edge from center while holding shift', () => {
    ...
    app.select('rect1').pointBoundsHandle(TLBoundsEdge.Left, { x: 0, y: 0 }).movePointer({ x: 20, y: 10, shiftKey: true, altKey: true }).completeSession();
  ...
  it('resizes corner from center', () => {
    ...
    app.select('rect1').pointBoundsHandle(TLBoundsCorner.TopLeft, { x: 0, y: 0 }).movePointer({ x: 20, y: 10, altKey: true }).completeSession();
  ...
  it('resizes corner from center while holding shift', () => {
    ...
    app.select('rect1').pointBoundsHandle(TLBoundsCorner.TopLeft, { x: 0, y: 0 }).movePointer({ x: 20, y: 10, shiftKey: true, altKey: true }).completeSession();
