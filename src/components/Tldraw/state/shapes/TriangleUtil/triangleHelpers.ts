...
import { getOffsetPolygon, getShapeStyle } from '@tldr/state/shapes/shared';
...
function getTriangleDrawPoints(id: string, size: number[], strokeWidth: number) {
  ...
  // Corners
  const corners = [Vec.add([w / 2, 0], offsets[0]), Vec.add([w, h], offsets[1]), Vec.add([0, h], offsets[2])];
  ...
  const lines = Utils.rotateArray(
    [Vec.pointsBetween(corners[0], corners[1], 32), Vec.pointsBetween(corners[1], corners[2], 32), Vec.pointsBetween(corners[2], corners[0], 32)],
    rm,
  );
