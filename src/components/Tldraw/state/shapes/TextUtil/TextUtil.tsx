...
export class TextUtil extends TDShapeUtil<T, E> {
  ...
  Component = TDShapeUtil.Component<T, E, TDMeta>(({ shape, isBinding, isGhost, isEditing, onShapeBlur, onShapeChange, meta, events }, reference) => {
  ...
  shouldRender = (previous: T, next: T): boolean => {
    return next.text !== previous.text || next.style.scale !== previous.style.scale || next.style !== previous.style;
  };
