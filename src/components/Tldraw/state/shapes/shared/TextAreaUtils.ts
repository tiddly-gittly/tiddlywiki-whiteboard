...
type ReplacerCallback = (substring: string, ...arguments_: unknown[]) => string;

const INDENT = '  ';

export const TextAreaUtils = {
  insertTextFirefox(field: HTMLTextAreaElement | HTMLInputElement, text: string): void {
  ...
  insert(field: HTMLTextAreaElement | HTMLInputElement, text: string): void {
  ...
  set(field: HTMLTextAreaElement | HTMLInputElement, text: string): void {
    field.select();
    TextAreaUtils.insert(field, text);
  },

  /** Get the selected text in a field or an empty string if nothing is selected. */
  getSelection(field: HTMLTextAreaElement | HTMLInputElement): string {
    const { selectionStart, selectionEnd } = field;
    return field.value.slice(selectionStart || undefined, selectionEnd || undefined);
  },
  ...
  wrapSelection(field: HTMLTextAreaElement | HTMLInputElement, wrap: string, wrapEnd?: string): void {
    const { selectionStart, selectionEnd } = field;
  ...
  replace(field: HTMLTextAreaElement | HTMLInputElement, searchValue: string | RegExp, replacer: string | ReplacerCallback): void {
    /** Remembers how much each match offset should be adjusted */
    let drift = 0;

    field.value.replace(searchValue, (...arguments_): string => {
      // Select current match to replace it later
      const matchStart = drift + (arguments_[arguments_.length - 2] as number);
  ...
  findLineEnd(value: string, currentEnd: number): number {
  ...
  indent(element: HTMLTextAreaElement): void {
  ...
  unindent(element: HTMLTextAreaElement): void {
    const { selectionStart, selectionEnd, value } = element;
    ...
    const difference = firstLineIndentation == undefined ? 0 : firstLineIndentation[0].length;
