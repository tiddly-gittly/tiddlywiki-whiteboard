...
export class PolygonUtils {
  ...
  static appendArc(polygon: number[][], center: number[], radius: number, startVertex: number[], endVertex: number[], isPaddingBoundary = false) {
  ...
  static createPaddingPolygon(polygon: number[][][], shapePadding = 0) {
    ...
    for (let i = 0; i < offsetEdges.length; i++) {
      ...
      if (vertex) vertices.push(vertex);
        ...
        PolygonUtils.appendArc(vertices, polygon[i][0], shapePadding, prevEdge[1], thisEdge[0], true);
      }
    }
