...
export function getTextSvgElement(text: string, fontSize: number, fontFamily: string, textAlign: AlignStyle, width: number, wrap = false) {
  ...
  const textLines = lines.map((line, index) => {
    ...
    tspan.setAttribute('y', lineHeight * fontSize * (index + 0.5) + 'px');
...
function breakText(options: {
  ...
  textElm.style.setProperty('width', options.width + 'px');
  textElm.style.setProperty('height', 'min-content');
  textElm.style.setProperty('font-size', options.fontSize + 'px');
  textElm.style.setProperty('font-family', options.fontFamily);
  textElm.style.setProperty('font-weight', options.fontWeight);
  textElm.style.setProperty('line-height', options.lineHeight * options.fontSize + 'px');
  textElm.style.setProperty('letter-spacing', options.letterSpacing);
  textElm.style.setProperty('text-align', options.textAlign);
  ...
  let previousHeight = textElm.offsetHeight;
