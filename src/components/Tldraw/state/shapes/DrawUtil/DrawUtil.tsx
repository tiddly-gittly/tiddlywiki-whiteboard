...
import { intersectBoundsBounds, intersectBoundsPolyline, intersectLineSegmentBounds, intersectLineSegmentLineSegment } from '@tldraw/intersect';
import { Vec } from '@tldraw/vec';
import * as React from 'react';
import { GHOSTED_OPACITY } from '@tldr/constants';
import { TDShapeUtil } from '@tldr/state/shapes/TDShapeUtil';
import { defaultStyle, getShapeStyle } from '@tldr/state/shapes/shared';
import { DashStyle, DrawShape, TDMeta, TDShapeType, TransformInfo } from '@tldr/types';
...
export class DrawUtil extends TDShapeUtil<T, E> {
  ...
  Component = TDShapeUtil.Component<T, E, TDMeta>(({ shape, meta, isSelected, isGhost, events }, reference) => {
    ...
    if (verySmall) {
      ...
      return (
        <SVGContainer ref={reference} id={shape.id + '_svg'} {...events}>
          <circle r={sw} fill={stroke} stroke={stroke} pointerEvents="all" opacity={isGhost ? GHOSTED_OPACITY : 1} />
        </SVGContainer>
      );
    }
      ...
      return (
        <SVGContainer ref={reference} id={shape.id + '_svg'} {...events}>
          <g opacity={isGhost ? GHOSTED_OPACITY : 1}>
            <path className={shouldFill || isSelected ? 'tl-fill-hitarea' : 'tl-stroke-hitarea'} d={pathTDSnapshot} />
            {shouldFill && <path d={polygonPathTDSnapshot} stroke="none" fill={fill} strokeLinejoin="round" strokeLinecap="round" pointerEvents="none" />}
            <path
              d={pathTDSnapshot}
              fill={stroke}
              stroke={stroke}
  ...
  shouldRender = (previous: T, next: T) => {
    return next.points !== previous.points || next.style !== previous.style || next.isComplete !== previous.isComplete;
  };
  ...
  hitTestBounds = (shape: T, bounds: TLBounds) => {
    ...
    if (!shape.rotation) {
      ...
      return (
        ...
        ((Utils.boundsContain(shapeBounds, bounds) || intersectBoundsBounds(shapeBounds, bounds).length > 0) &&
          intersectBoundsPolyline(Utils.translateBounds(bounds, Vec.neg(shape.point)), shape.points).length > 0)
      );
    }
