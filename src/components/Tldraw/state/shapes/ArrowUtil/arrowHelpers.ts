...
export function getBendPoint(handles: ArrowShape['handles'], bend: number) {
  ...
  const distribution = Vec.dist(start.point, end.point);

  const midPoint = Vec.med(start.point, end.point);

  const bendDistribution = (distribution / 2) * bend;

  const u = Vec.uni(Vec.vec(start.point, end.point));

  const point = Vec.toFixed(Math.abs(bendDistribution) < 10 ? midPoint : Vec.add(midPoint, Vec.mul(Vec.per(u), bendDistribution)));
