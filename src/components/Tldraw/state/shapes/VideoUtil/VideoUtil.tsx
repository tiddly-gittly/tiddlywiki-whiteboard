...
export class VideoUtil extends TDShapeUtil<T, E> {
  ...
  Component = TDShapeUtil.Component<T, E, TDMeta>(({ shape, asset = { src: '' }, isBinding, isEditing, isGhost, meta, events, onShapeChange }, reference) => {
  ...
  shouldRender = (previous: T, next: T) => {
    return next.size !== previous.size || next.style !== previous.style || next.isPlaying !== previous.isPlaying;
  };
