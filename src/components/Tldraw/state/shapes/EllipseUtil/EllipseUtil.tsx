...
import { intersectEllipseBounds, intersectLineSegmentEllipse, intersectRayEllipse } from '@tldraw/intersect';
import { Vec } from '@tldraw/vec';
import * as React from 'react';
import { GHOSTED_OPACITY, LABEL_POINT } from '@tldr/constants';
import { TDShapeUtil } from '@tldr/state/shapes/TDShapeUtil';
import { TextLabel, defaultStyle, getFontStyle, getShapeStyle } from '@tldr/state/shapes/shared';
import { styled } from '@tldr/styles';
import { DashStyle, EllipseShape, TDMeta, TDShape, TDShapeType, TransformInfo } from '@tldr/types';
...
export class EllipseUtil extends TDShapeUtil<T, E> {
  ...
  Component = TDShapeUtil.Component<T, E, TDMeta>(
    ({ shape, isGhost, isSelected, isBinding, isEditing, meta, bounds, events, onShapeChange, onShapeBlur }, reference) => {
  ...
  Indicator = TDShapeUtil.Indicator<T, M>(({ shape }) => {
    ...
    const rx = Math.max(0, radius[0] - sw / 2);
    const ry = Math.max(0, radius[1] - sw / 2);
    return style.dash === DashStyle.Draw ? <path d={getEllipseIndicatorPath(id, radius, style)} /> : <ellipse cx={radius[0]} cy={radius[1]} rx={rx} ry={ry} />;
  });

  hitTestPoint = (shape: T, point: number[]): boolean => {
    return (
      Utils.pointInBounds(point, this.getRotatedBounds(shape)) &&
      Utils.pointInEllipse(point, this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0)
    );
  };
  ...
  hitTestBounds = (shape: T, bounds: TLBounds): boolean => {
    ...
    return (
      ...
      intersectEllipseBounds(this.getCenter(shape), shape.radius[0], shape.radius[1], shape.rotation || 0, bounds).length > 0
    );
  };

  shouldRender = (previous: T, next: T): boolean => {
    return next.radius !== previous.radius || next.style !== previous.style || next.label !== previous.label;
  };
  ...
  getBindingPoint = <K extends TDShape>(shape: T, fromShape: K, point: number[], origin: number[], direction: number[], bindAnywhere: boolean) => {
    {
      ...
      if (bindAnywhere) {
        if (Vec.dist(point, this.getCenter(shape)) < 12) {
          ...
          bindingPoint = Vec.divV(Vec.sub(point, [expandedBounds.minX, expandedBounds.minY]), [expandedBounds.width, expandedBounds.height]);
        }
        distance = 0;
      } else {
        let intersection = intersectRayEllipse(origin, direction, center, shape.radius[0], shape.radius[1], shape.rotation || 0).points.sort(
          (a, b) => Vec.dist(a, origin) - Vec.dist(b, origin),
        )[0];
        if (!intersection) {
          intersection = intersectLineSegmentEllipse(point, center, center, shape.radius[0], shape.radius[1], shape.rotation || 0).points.sort(
        ...
        if (Vec.distanceToLineSegment(point, anchor, this.getCenter(shape)) < 12) {
          ...
          bindingPoint = Vec.divV(Vec.sub(anchor, [expandedBounds.minX, expandedBounds.minY]), [expandedBounds.width, expandedBounds.height]);
        }
