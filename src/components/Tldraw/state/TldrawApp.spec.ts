...
import { ArrowShape, ColorStyle, SessionType, TDShapeType } from '@tldr/types';
...
describe('TldrawTestApp', () => {
  describe('When copying and pasting...', () => {
    ...
    it('pastes a shape', () => {
      ...
      const previousCount = Object.keys(app.page.shapes).length;

      app.selectNone().copy(['rect1']).paste();

      expect(Object.keys(app.page.shapes).length).toBe(previousCount + 1);

      app.undo();

      expect(Object.keys(app.page.shapes).length).toBe(previousCount);
    ...
    it('Copies grouped shapes.', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).group(['rect1', 'rect2'], 'groupA').select('groupA').copy();
  ...
  describe('When copying and pasting a shape with bindings', () => {
    it('copies two bound shapes and their binding', () => {
      ...
      expect(newArrow.handles.start.bindingId).not.toBe(app.getShape<ArrowShape>('arrow1').handles.start.bindingId);
  ...
  describe('Selection', () => {
    ...
    it('selects and deselects a shape', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).selectNone().clickShape('rect1').clickCanvas();
      expect(app.selectedIds).toStrictEqual([]);
      expect(app.status).toBe('idle');
    });

    it('selects multiple shapes', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).selectNone().clickShape('rect1').clickShape('rect2', { shiftKey: true });
    ...
    it('clears selection when clicking bounds', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).selectAll().clickBounds().completeSession();
    ...
    it('does not select on meta-click', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).selectNone().clickShape('rect1', { ctrlKey: true }).expectSelectedIdsToBe([]);
    ...
    it('single-selects shape in selection on click', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).clickShape('rect1').clickShape('rect2', { shiftKey: true }).clickShape('rect2');
      expect(app.selectedIds).toStrictEqual(['rect2']);
      expect(app.status).toBe('idle');
    });

    it('single-selects shape in selection on pointerup only', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).clickShape('rect1').clickShape('rect2', { shiftKey: true }).pointShape('rect2');
      expect(app.selectedIds).toStrictEqual(['rect1', 'rect2']);
  ...
  describe('when selecting shapes in a group', () => {
    it('selects the group when a grouped shape is clicked', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).group(['rect1', 'rect2'], 'groupA').clickShape('rect1');
    ...
    it('clears the selectedGroupId when selecting a different shape', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).group(['rect1', 'rect2'], 'groupA').doubleClickShape('rect1').clickShape('rect3');
    ...
    it('selects a hovered shape from the selected group when meta-shift-clicked', () => {
      const app = new TldrawTestApp().loadDocument(mockDocument).group(['rect1', 'rect2'], 'groupA').clickShape('rect1', { ctrlKey: true, shiftKey: true });
  ...
  describe('when creating shapes', () => {
    it('Creates shapes with the correct child index', () => {
      ...
      const previousA = new Set(app.shapes.map((shape) => shape.id));

      app.pointCanvas({ x: 0, y: 0 }).movePointer({ x: 100, y: 100 }).stopPointing('canvas', [100, 100]);

      const newIdA = app.shapes.map((shape) => shape.id).find((id) => !previousA.has(id))!;
      ...
      const previousB = new Set(app.shapes.map((shape) => shape.id));

      app.pointCanvas({ x: 0, y: 0 }).movePointer({ x: 100, y: 100 }).stopPointing('canvas', [100, 100]);

      const newIdB = app.shapes.map((shape) => shape.id).find((id) => !previousB.has(id))!;
  ...
  describe('When copying to SVG', () => {
    it('Copies shapes.', async () => {
      const result = await new TldrawTestApp().loadDocument(mockDocument).select('rect1').rotate(0.1).selectAll().copySvg();
      expect(result).toMatchSnapshot('copied svg');
    });

    it('Copies grouped shapes.', async () => {
      const result = await new TldrawTestApp().loadDocument(mockDocument).select('rect1', 'rect2').group().selectAll().copySvg();

      expect(result).toMatchSnapshot('copied svg with group');
    });

    it('Respects child index', async () => {
      const result = await new TldrawTestApp().loadDocument(mockDocument).moveToBack(['rect2']).selectAll().copySvg();
...
describe('Arrow binding', () => {
  it('should delete handles bindingId', () => {
    ...
    app.select('arrow1').movePointer([200, 200]).startSession(SessionType.Arrow, 'arrow1', 'start').movePointer([1000, 1000]).completeSession();
