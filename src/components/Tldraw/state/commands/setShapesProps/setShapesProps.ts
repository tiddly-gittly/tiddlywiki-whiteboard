...
export function setShapesProps<T extends TDShape>(app: TldrawApp, ids: string[], partial: Partial<T>): TldrawCommand {
  const { currentPageId, selectedIds } = app;

  const initialShapes = ids.map((id) => app.getShape<T>(id)).filter((shape) => (partial.isLocked ? true : !shape.isLocked));
