...
import { DistributeType, TDShape, TDShapeType, TldrawCommand } from '@tldr/types';

export function distributeShapes(app: TldrawApp, ids: string[], type: DistributeType): TldrawCommand {
...
function getDistributions(initialShapes: TDShape[], type: DistributeType) {
  ...
  const results: Array<{ id: string; next: number[]; prev: number[] }> = [];
  ...
  switch (type) {
    case DistributeType.Horizontal: {
      ...
      if (span > commonBounds.width) {
        ...
        const step = (right.center[0] - left.center[0]) / (length_ - 1);

        const x = left.center[0] + step;

        entriesToMove.forEach(({ id, point, bounds }, index) => {
          results.push({
            id,
            prev: point,
            next: [x + step * index - bounds.width / 2, bounds.minY],
          });
        });
      } else {
        ...
        const step = (commonBounds.width - span) / (length_ - 1);
    ...
    case DistributeType.Vertical: {
      ...
      if (span > commonBounds.height) {
        ...
        const step = (bottom.center[1] - top.center[1]) / (length_ - 1);
        ...
        entriesToMove.forEach(({ id, point, bounds }, index) => {
          results.push({
            id,
            prev: point,
            next: [bounds.minX, y + step * index - bounds.height / 2],
          });
        });
      } else {
        ...
        const step = (commonBounds.height - span) / (length_ - 1);
