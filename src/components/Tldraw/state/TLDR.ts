...
import {
  ArrowShape,
  ShapesWithProp as ShapesWithProperty,
  TDBinding,
  TDExportType,
  TDHandle,
  TDPage,
...
export class TLDR {
  ...
  static getLinkedShapeIds(data: TDSnapshot, pageId: string, direction: 'center' | 'left' | 'right', includeArrows = true) {
    ...
    while (idsToCheck.length > 0) {
      ...
      arrows.forEach((arrow) => {
        ...
        const startBinding = startBindingId ? page.bindings[startBindingId] : undefined;
        const endBinding = endBindingId ? page.bindings[endBindingId] : undefined;
  ...
  static mutateShapes<T extends TDShape>(
    data: TDSnapshot,
    ids: string[],
    function_: (shape: T, index: number) => Partial<T> | undefined,
    pageId: string,
  ...
  static onChildrenChange<T extends TDShape>(data: TDSnapshot, shape: T, pageId: string) {
    ...
    const delta = TLDR.getShapeUtil(shape).onChildrenChange?.(
      shape,
      shape.children.map((id) => TLDR.getShape<T>(data, id, pageId)),
    );
  ...
  static getRotatedShapeMutation<T extends TDShape>(
    ...
    if (shape.handles !== undefined) {
      const change = this.getShapeUtil<T>(shape).onHandleChange?.(
  ...
  static assertShapeHasProperty<P extends keyof TDShape>(shape: TDShape, property: P): asserts shape is ShapesWithProperty<P> {
    if (shape[property] === undefined) {
      throw new Error(`shape[${property}] === undefined`);
    }
  }

  static warn(error: Error) {
    if (isDevelopment) {
      console.warn(error);
    }
  }
