...
export const TLDR = {
  getShapeUtil<T extends TDShape>(type: T['type']): TDShapeUtil<T>,
  getShapeUtil<T extends TDShape>(shape: T): TDShapeUtil<T>,
  getShapeUtil<T extends TDShape>(shape: T | T['type']) {
    return getShapeUtil<T>(shape)
  },

  getSelectedShapes(data: TDSnapshot, pageId: string) {
    const page = TLDR.getPage(data, pageId)
    const selectedIds = TLDR.getSelectedIds(data, pageId)
    return selectedIds.map((id) => page.shapes[id])
  },

  screenToWorld(data: TDSnapshot, point: number[]) {
  ...
  getPage(data: TDSnapshot, pageId: string): TDPage {
    return data.document.pages[pageId]
  },

  getPageState(data: TDSnapshot, pageId: string): TLPageState {
    return data.document.pageStates[pageId]
  },

  getSelectedIds(data: TDSnapshot, pageId: string): string[] {
    return TLDR.getPageState(data, pageId).selectedIds
  },

  getShapes(data: TDSnapshot, pageId: string): TDShape[] {
    return Object.values(TLDR.getPage(data, pageId).shapes)
  },

  getCamera(data: TDSnapshot, pageId: string): TLPageState['camera'] {
  ...
  getCenter<T extends TDShape>(shape: T) {
    return TLDR.getShapeUtil(shape).getCenter(shape)
  },

  getBounds<T extends TDShape>(shape: T) {
    return TLDR.getShapeUtil(shape).getBounds(shape)
  },

  getRotatedBounds<T extends TDShape>(shape: T) {
    return TLDR.getShapeUtil(shape).getRotatedBounds(shape)
  },

  getSelectedBounds(data: TDSnapshot): TLBounds {
  ...
  getParentId(data: TDSnapshot, id: string, pageId: string) {
  ...
  getDocumentBranch(data: TDSnapshot, id: string, pageId: string): string[] {
    const shape = TLDR.getShape(data, id, pageId)
  ...
  getSelectedBranchSnapshot(data: TDSnapshot, pageId: string): TDShape[],
  getSelectedBranchSnapshot<K>(
    data: TDSnapshot,
    pageId: string,
    fn?: (shape: TDShape) => K
  ...
  getSelectedShapeSnapshot(data: TDSnapshot, pageId: string): TDShape[],
  ...
  getAllEffectedShapeIds(data: TDSnapshot, ids: string[], pageId: string): string[] {
    const page = TLDR.getPage(data, pageId)
  ...
  getChildIndexAbove(data: TDSnapshot, id: string, pageId: string): number {
  ...
  getBeforeShape<T extends TDShape>(shape: T, change: Partial<T>): Partial<T> {
    return Object.fromEntries(
      Object.keys(change).map((k) => [k, shape[k as keyof T]])
    ) as Partial<T>
  },

  mutateShapes<T extends TDShape>(
    data: TDSnapshot,
    ids: string[],
  ...
  createShapes(data: TDSnapshot, shapes: TDShape[], pageId: string): TldrawCommand {
  ...
  onSessionComplete<T extends TDShape>(shape: T) {
    const delta = TLDR.getShapeUtil(shape).onSessionComplete?.(shape)
    if (!delta) return shape
    return { ...shape, ...delta }
  },

  onChildrenChange<T extends TDShape>(data: TDSnapshot, shape: T, pageId: string) {
  ...
  updateArrowBindings(page: TDPage, arrowShape: ArrowShape) {
    const result = {
  ...
  transform<T extends TDShape>(shape: T, bounds: TLBounds, info: TLTransformInfo<T>) {
    const delta = TLDR.getShapeUtil(shape).transform(shape, bounds, info)
    if (!delta) return shape
    return { ...shape, ...delta }
  },

  transformSingle<T extends TDShape>(shape: T, bounds: TLBounds, info: TLTransformInfo<T>) {
   ...
   */
  getRotatedShapeMutation<T extends TDShape>(
  ...
  updateParents(data: TDSnapshot, pageId: string, changedShapeIds: string[]): void {
    const page = TLDR.getPage(data, pageId)
  ...
  getBinding(data: TDSnapshot, id: string, pageId: string): TDBinding {
    return TLDR.getPage(data, pageId).bindings[id]
  },

  getBindings(data: TDSnapshot, pageId: string): TDBinding[] {
    const page = TLDR.getPage(data, pageId)
    return Object.values(page.bindings)
  },

  getBindableShapeIds(data: TDSnapshot) {
  ...
  getBindingsWithShapeIds(data: TDSnapshot, ids: string[], pageId: string): TDBinding[] {
    return Array.from(
      new Set(
  ...
  getRelatedBindings(data: TDSnapshot, ids: string[], pageId: string): TDBinding[] {
  ...
  flattenShape : (data: TDSnapshot, shape: TDShape): TDShape[] => {
    return [
      shape,
      ...(shape.children ?? [])
  ...
  assertShapeHasProperty<P extends keyof TDShape>(
    shape: TDShape,
    prop: P
  ...
  getSvgString(svg: SVGElement, scale = 1) {
  ...
  getSvgAsDataUrl(svg: SVGElement, scale = 1) {
